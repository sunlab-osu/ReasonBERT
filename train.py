...
def main(config, args):
    ...
    if 'SpanBERT' in config['arch']['args']['bert_version']:
    ...
    if config['train_dataset']['args'].get('dataset', None) is None:
        train_dataset = config.init_obj('train_dataset', module_data,\
            tokenizer=tokenizer, dataset=config["dataset"])
    else:
        train_dataset = config.init_obj('train_dataset', module_data,\
            tokenizer=tokenizer)
    ...
    if config['eval_dataset']['args'].get('dataset', None) is None:
        eval_dataset = config.init_obj('eval_dataset', module_data,\
            tokenizer=tokenizer, dataset=config["dataset"], sample_num=4096)
    else:
        eval_dataset = config.init_obj('eval_dataset', module_data,\
            tokenizer=tokenizer, sample_num=4096)
    if config['do_test'] != 0:
        try:
    ...
    if config['local_pretrained'] and config['use_our_pretrained']==1:
        ...
        pretrained_model = torch.load(os.path.join(config['local_pretrained'], 'pytorch_model.bin'))
        state_dict = model.state_dict()
        is_mlm_model = any(['lm_head' in k or 'cls.predictions' in k for k in pretrained_model.keys()])
        model.bert.resize_token_embeddings(model.bert.config.vocab_size+1) # handle extra special token [QUESTION]
        not_used = []
        ...
        del state_dict
    model.bert.resize_token_embeddings(len(train_dataset.tokenizer)) # handle extra special tokens like [DOC]

    if config.resume is not None:
        last_trained = torch.load(os.path.join(config.resume.resolve().as_posix(), 'pytorch_model.bin'))
        model.load_state_dict(last_trained)
        del last_trained
    
    if config['train_dataset']['args']['sample_num']<1 and config['train_dataset']['args']['sample_num']!=-1:
        config['train_dataset']['args']['sample_num'] = len(train_dataset)
    real_batch_size = config['trainer']["per_device_train_batch_size"]*config['trainer']["gradient_accumulation_steps"]*config['n_gpus']
    step_per_epoch = int(len(train_dataset)/real_batch_size)
    # Set the training steps, mostly for fewshot settings
    if config['train_dataset']['args']['sample_num']<=2048 and config['train_dataset']['args']['sample_num']!=-1:
    ...
    collator = config.init_obj('collator', module_data, tokenizer=train_dataset.tokenizer)
    trainer = config.init_obj('trainer_type', module_trainer,
    ...
    if config['eval_dataset']['args'].get('dataset', None) is None:
        full_eval_dataset = config.init_obj('eval_dataset', module_data,\
            tokenizer=tokenizer, dataset=config["dataset"])
    else:
        full_eval_dataset = config.init_obj('eval_dataset', module_data,\
            tokenizer=tokenizer)
    result_f = open(os.path.join(config.save_dir.resolve().as_posix(), 'results.json'), 'w')
    eval_output = trainer.predict(full_eval_dataset)
    logger.info(f"----Eval Results-----\n{eval_output.metrics}")
    result_f.write(f"----Eval Results-----\n{eval_output.metrics}\n")
    with open(os.path.join(config.save_dir.resolve().as_posix(), 'eval_predictions.json'), 'w') as f:
        json.dump(eval_output.predictions, f)
    if config['do_test'] != 0:
        test_output = trainer.predict(test_dataset)
        logger.info(f"-----Test Results-----\n{test_output.metrics}")
        result_f.write(f"-----Test Results-----\n{test_output.metrics}\n")
        with open(os.path.join(config.save_dir.resolve().as_posix(), 'test_predictions.json'), 'w') as f:
            json.dump(test_output.predictions, f)
    try:
        if config['do_train'] != 0:
            train_output = trainer.predict(train_dataset)
            logger.info(f"-----Train Results-----\n{train_output.metrics}")
            with open(os.path.join(config.save_dir.resolve().as_posix(), 'train_predictions.json'), 'w') as f:
...
if __name__ == '__main__':
    ...
    args.add_argument('--run_id', default=0, type=str,
                      help='run id')
    args.add_argument('--use_all', action='store_true',
                      help='use all pretrained headers')
    args.add_argument('--overwrite', action='store_true')

    # custom cli options to modify configuration from default values given in json file.
    CustomArgs = collections.namedtuple('CustomArgs', 'flags type target')
    options = [
        CustomArgs(['--lr', '--learning_rate'], type=float, target='trainer;learning_rate'),
        ...
        CustomArgs(['--bs', '--batch_size'], type=int, target='trainer;per_device_train_batch_size'),
        CustomArgs(['--acc'], type=int, target='trainer;gradient_accumulation_steps'),
        CustomArgs(['--seed', '--random_seed'], type=int, target='seed'),
        CustomArgs(['--sample', '--train_sample_num'], type=float, target='train_dataset;args;sample_num'),
        ...
        CustomArgs(['--pretrain', '--pretrain_model_type'], type=str, target='arch;args;bert_version'),
