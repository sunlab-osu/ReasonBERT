    ...
    def compute_loss(self, model, inputs):
        ...
        if isinstance(loss, dict):
            ...
            try:
                ...
                self.tmp_sub_loss = {k:torch.tensor(0.0).to(self.args.device) for k in loss if k!='loss'}
...
class BaseTrainerForQA(Trainer):
    ...
    def prediction_loop(self, dataloader: DataLoader, description: str, prediction_loss_only: Optional[bool] = None, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = 'eval') -> PredictionOutput:
        ...
        for step, inputs in enumerate(dataloader):
            loss, preds = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            for i, pred in enumerate(zip(*preds)):
                ...
                pred_answer = dataloader.dataset.tokenizer.decode(inputs['input_ids'][i][pred[0].item():pred[1].item()+1])
                if pred_answer not in q_results[qid]['preds'] or q_results[qid]['preds'][pred_answer]<pred[2].item():
        ...
        for qid, result in q_results.items():
            pred = sorted(list(result['preds'].items()), reverse=True, key=lambda z:z[1])[0]
            pred_answer = pred[0] + ' [DOC] [PAR] [TLE] [SEP]'
            special_tok_loc = min([pred_answer.find('[DOC]'), pred_answer.find('[PAR]'), pred_answer.find('[TLE]'), pred_answer.find('[SEP]')])
            pred_answer = pred_answer[:special_tok_loc].strip()
            answers = result['answers']
            result['pred_answer'] = pred_answer
            predictions[qid] = [pred_answer, pred[1]]
            f1.append(metric.metric_max_over_ground_truths(metric.f1_score, pred_answer, answers))
            exact_match.append(metric.metric_max_over_ground_truths(metric.exact_match_score, pred_answer, answers))
        f1 = 100.0*np.mean(f1)
        ...
        return PredictionOutput(predictions=predictions, metrics=metrics, label_ids=None)
    ...
    def prediction_loop(self, dataloader: DataLoader, description: str, prediction_loss_only: Optional[bool] = None, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = 'eval') -> PredictionOutput:
        ...
        for qid, result in q_results.items():
            ...
            for i, (cid, _) in enumerate(preds[:5]):
                ...
                if i==1:
                    top2_fact_recall.append(len(facts_found&all_facts)/len(all_facts))
                    top2_answer_recall.append(1 if found_answer else 0)
                elif i==2:
                    top3_fact_recall.append(len(facts_found&all_facts)/len(all_facts))
                    top3_answer_recall.append(1 if found_answer else 0)
                elif i==4:
                    top5_fact_recall.append(len(facts_found&all_facts)/len(all_facts))
...
class BaseTrainerForCS(Trainer):
    ...
    def compute_loss(self, model, inputs):
        loss = model(
            ...
            target_start_loc=inputs['target_start'],
            target_mask=inputs['target_mask'],
            target_start_onehot=inputs['target_start_onehot'] if 'target_start_onehot' in inputs else None,
    ...
    def prediction_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], prediction_loss_only: bool, ignore_keys: Optional[List[str]] = None):
        ...
        with torch.no_grad():
            ...
            if len(result)==2:
                ...
                start_p = start_pred_logits.exp()
                start_p[:,:,0] = 0
                max_p, pred_loc = torch.max(start_p.reshape(start_p.shape[0], -1), dim=-1)
            else:
                loss, row_pred_logits, column_pred_logits = result
                row_pred_logits -= row_pred_logits[:,:,0:1]
                row_p = row_pred_logits.exp()
                row_p[:,:,0] = 0
                max_row_p, pred_row_loc = torch.max(row_p.reshape(row_p.shape[0], -1), dim=-1)
                column_pred_logits -= column_pred_logits[:,:,0:1]
                column_p = column_pred_logits.exp()
                column_p[:,:,0] = 0
                max_column_p, pred_column_loc = torch.max(column_p.reshape(column_p.shape[0], -1), dim=-1)
    ...
    def prediction_loop(self, dataloader: DataLoader, description: str, prediction_loss_only: Optional[bool] = None, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = 'eval') -> PredictionOutput:
        ...
        for step, inputs in enumerate(dataloader):
            ...
            for i, pred in enumerate(zip(*preds)):
                ...
                if cell_id not in q_results[qid]['preds'] or q_results[qid]['preds'][cell_id]<pred[-1].item():
        ...
        predictions = {}
        for qid, result in q_results.items():
            preds = sorted(list(result['preds'].items()), reverse=True, key=lambda z:z[1])
            pred_cells = [pred[0] for pred in preds]
            answers = result['answers']
            predictions[qid] = preds[:10]
            exact_match.append(1 if pred_cells[0] in answers else 0)
            top2_acc.append(1 if any([pred_cell in answers for pred_cell in pred_cells[:2]]) else 0)
            top3_acc.append(1 if any([pred_cell in answers for pred_cell in pred_cells[:3]]) else 0)
            top5_acc.append(1 if any([pred_cell in answers for pred_cell in pred_cells[:5]]) else 0)
        exact_match = 100.0*np.mean(exact_match)
